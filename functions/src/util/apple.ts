...
import {ApplePage, AppleSong} from "../types/appleSong";

const extractItems = (response: ApplePage): AppleSong[] => {
    const lib = response.resources["library-songs"];
    return response.data.map((song: { id: string }): AppleSong => lib[song.id]);
};

const fetchAllPages = async (url: string, headers: { [key: string]: string }) => {
    const items = [];
    let page: ApplePage = await (await fetch(url, {headers})).json() as ApplePage;
    ...
    while (page.data.length && page.meta.total > items.length) {
        ...
        page = await (await fetch(url + "&offset=" + items.length, {headers})).json() as ApplePage;
        items.push(...extractItems(page));
    }
    return items;
};

const mapSong = (song: AppleSong): GenericSong => ({
    ...
    albumArt: song?.attributes?.artwork?.url?.
        ...
        replace("{h}", song.attributes.artwork.height.toString()).replace("{f}", "jpg") ?? null,
